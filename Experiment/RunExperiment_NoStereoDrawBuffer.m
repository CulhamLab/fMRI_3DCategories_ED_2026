%RunExperiment_NoStereoDrawBuffer(type,par,run)
%
%Important Note:
%   If you change parameters, run "ClearGlobal" to ensure that loading uses
%   the new settings.
%
% This version manually draws each view with/without the blue line instead
% of using StereoDrawBuffer.
%
function RunExperiment_NoStereoDrawBuffer(par,run)
...
%stereo
p.SCREEN.INVERT_BLUELINE = false; % if true, then the blue line is drawn for left instead of right (enable if views are incorrect)
...
%video
p.VIDEOS.MEMORY_SAVING_FACTOR = 1.0;    %ratio of frame size to store
...
p.VIDEOS.FLIP_HORIZONTAL = p.IMAGES.FLIP_HORIZONTAL;
...
while 1
    ...
    DrawFormattedText(s.win, msg, 'center', 80, p.SCREEN.TEXT_COLOUR);
    if d.display_events.Events{1}.Fixation(1)
        DrawFixation(p,s,"left")
    end
    drawBlueLine(p, s, "left")
    Screen('flip', s.win);
    ...
    DrawFormattedText(s.win, msg, 'center', 80, p.SCREEN.TEXT_COLOUR);
    if d.display_events.Events{1}.Fixation(1)
        DrawFixation(p,s,"right")
    end
    drawBlueLine(p, s, "right")
    Screen('flip', s.win);
...
function DrawFixation(p,s,view)
global g
switch lower(view)
    case "left"
...
function drawBlueLine(p, s, view)
...
switch lower(view)
    case "left"
        frame = ~p.SCREEN.INVERT_BLUELINE; % defaults to 1
    case "right"
        frame = p.SCREEN.INVERT_BLUELINE; % defaults to 0
    otherwise
        error("unsupported view: %s", view)
end
...
rowStart = [s.rect(1), s.rect(4)-1];
rowEnd = [s.rect(3), s.rect(4)-1];
Screen('DrawLine', s.win, frameColor, rowStart(1), rowStart(2), rowEnd(1), rowEnd(2),2);